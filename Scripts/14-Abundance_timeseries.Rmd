...
# Average and relative composition of abundance and biomass by bioregion and season
...
# png(here::here(paste0("Figures/Composition_abund_biom_season_",Sys.Date(),".png")),
...
# Mean total abundance and anomaly for spring (Apr - Jun)
```{r, fig.width=10, fig.height=8}
abund.ts <- zoop_sub %>% 
  left_join(dplyr::select(sp.table, Species, AssocGroup, AssocGroup.sub), 
            by = "Species") %>% 
  ...
  left_join(dplyr::select(zoopenvi, Key, DayOfYear), by = "Key") %>% 
  filter(Month <= 6) %>% 
  group_by(Key, Bioregion, Year ) %>% 
  summarise(Abundance = sum(Abundance.m2),
            Biomass = sum(Biomass.mg.m2)) %>% 
  group_by(Bioregion, Year) %>% 
  summarise(Abundance.m = mean(Abundance),
            Abundance.sd = sd(Abundance),
            Biomass.m = mean(Biomass),
            Biomass.sd = sd(Biomass),
            nsamples = n()) %>% 
  ungroup() 

g7 <- ggplot(abund.ts, aes(x = Year, y = Abundance.m, color = Bioregion)) +
  # geom_errorbar(aes(ymin = Abundance.m - Abundance.sd,
  #                   ymax = Abundance.m + Abundance.sd)) +
  geom_point(shape = 0) + 
  geom_line()+
...
abund.ts.anom <- abund.ts %>% 
  group_by(Bioregion) %>% 
  mutate(mabu = mean(Abundance.m),
         mbio = mean(Biomass.m)) %>% 
  mutate(Abundance.anom = Abundance.m - mean(Abundance.m),
         Biomass.anom = Biomass.m - mean(Biomass.m))

# Abundance
g8 <- ggplot(abund.ts.anom, aes(x=Year, y=Abundance.anom)) +
  ...
  theme(legend.title = element_text(color = 'black', size = 16, face = 'bold'),
        ...
        axis.text = element_text(size = 12),
        axis.text.x = element_text(angle = 90))+ xlab("Year") +
  ylab("Average Abundance Anomaly") +
  scale_x_continuous(breaks = seq(1995, 2014, by = 1))  +
  coord_cartesian(xlim = c(1995.5,2014)) +
  facet_wrap(~ Bioregion, ncol = 1, scales = "free_y", strip.position = "left")
...
png(here::here(paste0("Figures/Abundance_anomaly_timeseries_spring_",
                      Sys.Date(),".png")),
    width = 10.5, height = 8, units = "in", res = 300)
plot_grid(g7,g8,ncol=1, labels = "auto", rel_heights = c(1,2))
dev.off()
```


# Mean total abundance and anomaly for summer (Jul-Oct)
```{r, fig.width=10, fig.height=8}
abund.ts <- zoop_sub %>% 
  left_join(dplyr::select(sp.table, Species, AssocGroup, AssocGroup.sub), 
            by = "Species") %>% 
...
g9 <- ggplot(abund.ts, aes(x = Year, y = Abundance.m, color = Bioregion)) +
  # geom_errorbar(aes(ymin = Abundance.m - Abundance.sd,
  #                   ymax = Abundance.m + Abundance.sd)) +
  geom_point(shape = 0) + 
  geom_line()+
...
# Abundance
g10 <- ggplot(abund.ts.anom, aes(x=Year, y=Abundance.anom)) +
  geom_bar(stat = "identity") +
  theme(legend.title = element_text(color = 'black', size = 16, face = 'bold'),
        legend.text = element_text(size = 16),
        legend.position = "bottom",
...
png(here::here(paste0("Figures/Abundance_anomaly_timeseries_summer_",
                      Sys.Date(),".png")),
    width = 10.5, height = 8, units = "in", res = 300)
plot_grid(g9,g10,ncol=1, labels = "auto", rel_heights = c(1,2))
dev.off()
```
