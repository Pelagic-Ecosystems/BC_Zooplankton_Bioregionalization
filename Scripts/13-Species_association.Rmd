...
In this species association analysis, a concordance analysis is done with the assumption that species association are hierarchically structured. The log-transformed abundances are correlated between species and the correlation matrix is then converted to a distance matrix for clustering. 
...
for (k in c(3:10)) {
  dend.hel <- fviz_dend(assoc.clust, cex = 0.6, k = k,color_labels_by_k = TRUE, hang = 10, 
...
Dendrogram with colored nodes according to bioregion with highest IndVal.
...
NMDS plots for association group level
```{r, fig.height=11, fig.width=11}
...
Indval nmds colored by bioregion assignment in indval. For possible subplot overlayed at upper rightside of assoc group nmds
