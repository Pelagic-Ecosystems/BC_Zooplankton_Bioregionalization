...
# Load the data
zoop <-  read_csv(here::here("Data_Raw/Zoop_BC_08122020_clean.csv"), col_types = "fffffffffffffffnnnnnnnnnnnnnnnnnnn" ) # This is the concatenated DFO csv files without notes column
zoop <-  as_tibble(zoop)
...
Based on project discussion on April 2021, various tow ratio thresholds were explored and the group decided to remove samples with Tow.Ratio < 70% of bottom depth when tow depth was < 100 m. 

Note that instances of low tow ratios when the tow depth are ~150m and ~250m.
```{r}
red <- zoop_sub %>%
  dplyr::select(Key,Station,Project,Depth_Start,`Depth_Bottom(m)`,Tow.Ratio,
                Longitude,Latitude,Day,Month,Year,dtsample, Num_Species) %>% 
...
# rm.3 <- which(red$Tow.Ratio < 0.7 & red$Depth_Start <= 115)
